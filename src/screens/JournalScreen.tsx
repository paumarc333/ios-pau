...
// Text is rendered as a single block (whiteSpace: pre-wrap); only the whole
// bubble animates in — never split into per-word spans.
...
${tasksStr}`
      ...
      if (text) {
        ...
        await Promise.all([
          ...
          supabase.from('user_settings').upsert({ key: `journal_retro_${key}`, value: cache }, { onConflict: 'key' }),
        ])
        setSummaryHistory(prev => {
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {summaryHistory.map(({ month, text }) => {
                    ...
                    return (
                      ...
                      >
                        <button
                          onClick={() => setOpenHistoryMonth(isOpen ? null : month)}
                          style={{
                          ...
                          {isOpen && (
                            ...
                            >
                              <div style={{ padding: '0 14px 14px', borderTop: '0.5px solid rgba(139,92,246,0.1)', paddingTop: 12 }}>
                                ...
                                <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.65)', lineHeight: 1.75, whiteSpace: 'pre-wrap' }}>
                                  {text}
                                </div>
                              </div>
                            </motion.div>
